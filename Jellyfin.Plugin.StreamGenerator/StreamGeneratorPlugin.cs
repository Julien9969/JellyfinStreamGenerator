    ...
    public static string PatchContextMenu(PatchRequestPayload payload)
    {
        ...
        try
        {
            ...
            var generateStreamCase =
                @"case""generate-stream"":if(window.showStreamGeneratorPopup){window.showStreamGeneratorPopup(s,u)}else if(window.streamGeneratorPopupPromise){window.streamGeneratorPopupPromise.then(function(){if(window.showStreamGeneratorPopup)window.showStreamGeneratorPopup(s,u)}).catch(function(e){console.error(""StreamGenerator popup script failed to load"",e)})}else{console.error(""StreamGenerator popup script not loaded!"")}break;";
